...
class GLF(nn.Module):
    ...
    def __init__(self, c1):
        ...
        self.globalattention = nn.AdaptiveAvgPool2d(1)
        self.conv1 = nn.Conv2d(c1 * 2, c1 // 2, 1, 1)
        # self.act = nn.ReLU()
        self.conv2 = nn.Conv2d(c1 // 2, c1, 1, 1)
        self.sigmoid = nn.Sigmoid()
        self.localattention = nn.Conv2d(c1 * 2, c1, 3, 1, 1)
        self.final = nn.Conv2d(c1 * 3, c1, 3, 1, 1)

    def forward(self, x):
        x1 = torch.cat((x[0], x[1]), dim=1)
        out1 = self.sigmoid(self.conv2(self.conv1(self.globalattention(x1)))) * x[0]
        out2 = self.sigmoid(self.localattention(x1)) * x[0]
        out3 = self.final(torch.cat((out1, out2, x[0]), dim=1)) + x[0]
        return out3
...
class DenseDcnBottleneck(nn.Module):
    # Standard bottleneck
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c_, c2 // 2, 3, 1)
        self.cv3 = Conv(c2 // 2, c2 // 2, 3, 1)
        self.cv4 = DCNv2(c2 * 2, c2, 3, 1)
        #         self.cv5 = Conv(c2, (c2-c_)//2, 3, 1, g=g)
        self.add = shortcut and c1 == c2
    ...
    def forward(self, x):
        ...
        x2 = self.cv2(x1)
        x3 = self.cv3(x2)
        x4 = self.cv4(torch.cat((x1, x2, x3), dim=1))
        return x4 + x
...
class RFA(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.Sequential(*(DenseDcnBottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))
